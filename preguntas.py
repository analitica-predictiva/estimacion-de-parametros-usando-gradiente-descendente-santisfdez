...
def pregunta_02():
    ...
    for _ in range(n_iterations):
        ...
        gradient = np.array([-2*sum(error),
                            -2*sum([error_i*x_value_i for error_i, x_value_i in zip(error,x_poly[:,1])]),
                            -2*sum([error_i*x_value_i for error_i, x_value_i in zip(error,x_poly[:,2])])])
